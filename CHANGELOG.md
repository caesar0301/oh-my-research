...
- **Attribution text unified to "Powered by oh-my-research"**: the footer product credit no longer varies by report language (previously e.g. "Generated by oh-my-research" / "由 oh-my-research 生成" / "Généré par oh-my-research" per locale). `LANG_STRINGS` no longer carries a `generated` entry — `attribution_label()` returns the single `ATTRIBUTION_TEXT` constant (`attribution.text` still overrides it). Reference docs and the `_document.json` template note were updated to match.
